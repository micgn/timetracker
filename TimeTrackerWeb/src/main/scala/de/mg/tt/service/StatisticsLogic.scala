...
class StatisticsLogic {
  ...
  Map[(Int, Int), (Double, Int, Double, Double, Double)] = {
    ...
    val hourAveragePerPeriod = groupedActivities.map { case (period, _) =>
      (period, hourSumPerPeriod(period) / daysPerPeriod(period))
    }

    val eightHourDaysPerPeriod = groupedActivities.map { case (period, _) =>
      (period, hourSumPerPeriod(period) / 8.0)
    }

    val maxHoursPerPeriod = groupedActivities.map { case (period, activities) => (period, findMaxHoursPerDay(activities) ) }

    groupedActivities.map { case (period, _) => (period,
      (hourSumPerPeriod(period),
        daysPerPeriod(period),
        eightHourDaysPerPeriod(period),
        hourAveragePerPeriod(period),
        maxHoursPerPeriod(period)))
    }
  }
